...
class INIModifierApp:
    ...
    def __init__(self, root, config_dir: Path):
        self.root = root
        self.root.title("NTE Hidden Graphics Settings Tool")
        ...
        center_window(self.root, 480, 260)
    ...
    def init_files(self):
        ...
        if already_decrypted_flag:
            messagebox.showinfo(
                "Decryption Notice", "Your INI files appear to be decrypted already. Skipping decryption."
            )
    ...
    def setup_ui(self):
        ...
        btn_hidden = ttk.Button(
            main_frame,
            text="Add Hidden Graphics Settings",
        ...
        btn_export = ttk.Button(
            main_frame,
            text="Export Decrypted Config Files",
            command=self.export_decrypted_files,
        )
        btn_export.pack(fill=tk.X, pady=5)

        btn_restore = ttk.Button(
            main_frame, text="Restore Settings", command=self.restore_settings
        )
        ...
        ttk.Label(
            main_frame,
            text=f"Current Directory: {display_path}",
            foreground="gray",
            font=("Segoe UI", 8),
        ).pack(anchor=tk.W, pady=(15, 0))

    def export_decrypted_files(self):
        target_dir = filedialog.askdirectory(title="Select Location to Export Decrypted Config Files")
        if not target_dir:
            return
        ...
        try:
            ...
            messagebox.showinfo(
                "Export Successful",
                f"Decrypted config files saved to:\n{target_dir}",
            )
        except Exception as e:
            messagebox.showerror("Export Failed", f"Error saving config files:\n{e}")
                ...
                if bak_file.exists():
                    try:
                        ...
                        messagebox.showerror(
                            "Restore Failed", f"Error restoring {target_name}:\n{e}"
                        )

        if restored_files:
            messagebox.showinfo(
                "Restore Complete",
                "Successfully restored the following config files:\n" + "\n".join(restored_files),
            )
            self.init_files()
        else:
            messagebox.showwarning(
                "No Backup Found",
                "No matching .bak backup files found in target directory!",
            )

    def open_hidden_settings_menu(self):
        sub_window = tk.Toplevel(self.root)
        sub_window.withdraw()                                         
        sub_window.title("Add Hidden Graphics Settings")
        ...
        center_window(sub_window, 450, 280)
        ...
        ttk.Checkbutton(
            frame,
            text="HDR Settings *Enabled by default (no in-game menu)",
            variable=var_hdr,
        ).pack(anchor=tk.W, pady=6)

        ttk.Checkbutton(
            frame, text="Enable Lumen & Hardware Ray Tracing", variable=var_lumen
        ).pack(anchor=tk.W, pady=6)

        ttk.Checkbutton(
            frame, text="CPU Async Compute Optimization", variable=var_cpu
        ).pack(anchor=tk.W, pady=6)

        ttk.Checkbutton(
            frame,
            text="Unlock FPS Limit *Displaying 30 in menu is normal",
            variable=var_fps,
        ).pack(anchor=tk.W, pady=6)
        ...
        def apply_changes():
            if not (var_hdr.get() or var_lumen.get() or var_cpu.get() or var_fps.get()):
                messagebox.showwarning(
                    "Notice", "Please select at least one option!", parent=sub_window
                )
                return
            ...
            if (var_hdr.get() or var_lumen.get() or var_cpu.get()) and any(
                ...
                confirm = messagebox.askokcancel(
                    ...
                    "Engine.ini appears to be modified, which may cause conflicts.\n"
                    "If previously modified by this tool, restore settings in the main menu first.\n\n"
            ...
            try:
                ...
                if var_hdr.get() or var_lumen.get() or var_cpu.get():
                    ...
                    modified_summary.append("Engine.ini (Locked as Read-Only)")

                summary_str = " & ".join(modified_summary)
                messagebox.showinfo(
                    "Success",
                    f"Configuration successfully saved!\n\nModified: {summary_str}",
                    parent=sub_window,
                )
                sub_window.destroy()
                ...
                messagebox.showerror(
                    "Save Failed", f"Error writing configuration files:\n{e}", parent=sub_window
                )

        btn_confirm = ttk.Button(
            frame, text="Apply Changes", command=apply_changes
        )
...
def main():
    ...
    if not target_dir:       
        messagebox.showerror(
            "Directory Not Found",
            "Could not find config folder under AppData\\Local\\HT!\n"
            "Please make sure the game has been run to generate config files.",
        )
        sys.exit(0)
