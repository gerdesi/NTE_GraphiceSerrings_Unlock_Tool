...
class INIModifierApp:
    ...
    def setup_ui(self):
        ...
        btn_restore = ttk.Button(
            main_frame, text="还原设定", command=self.restore_settings
        )
        ...
        ttk.Label(
            main_frame,
            text=f"当前目录: {display_path}",
            foreground="gray",
            font=("Microsoft JhengHei", 8),
        ).pack(anchor=tk.W, pady=(15, 0))

    def export_decrypted_files(self):
        target_dir = filedialog.askdirectory(title="选择导出解密配置文件的位置")
        if not target_dir:
            return
        ...
        try:
            ...
            messagebox.showerror("导出失败", f"保存配置文件时发生错误：\n{e}")
    ...
    def open_hidden_settings_menu(self):
        ...
        ttk.Checkbutton(
            frame,
            text="HDR设定 *无游戏内菜单默认开启",
            variable=var_hdr,
        ).pack(anchor=tk.W, pady=6)

        ttk.Checkbutton(
            frame, text="开启Lumen和硬件光追", variable=var_lumen
        ).pack(anchor=tk.W, pady=6)

        ttk.Checkbutton(
            frame, text="CPU异步计算优化", variable=var_cpu
        ).pack(anchor=tk.W, pady=6)

        ttk.Checkbutton(
            frame,
            text="解锁fps上限 *设置内显示30是正常的",
            variable=var_fps,
        ).pack(anchor=tk.W, pady=6)
        ...
        def apply_changes():
            if not (var_hdr.get() or var_lumen.get() or var_cpu.get() or var_fps.get()):
                messagebox.showwarning(
                    "提示", "请至少勾选一个修改项！", parent=sub_window
                )
                return
            ...
            if (var_hdr.get() or var_lumen.get() or var_cpu.get()) and any(
                ...
                confirm = messagebox.askokcancel(
                    "配置冲突警告",
                    "似乎你解密并修改过 engine.ini，新的配置可能会冲突；\n"
                    "如果使用本工具修改过，请到主菜单还原设定后再继续。\n\n"
                    "是否继续更改？",
                    parent=sub_window,
                )
                if not confirm:
            ...
            try:
                ...
                if var_hdr.get() or var_lumen.get() or var_cpu.get():
                    ...
                    modified_summary.append("Engine.ini(已锁定只读)")
        ...
        btn_confirm = ttk.Button(
            frame, text="应用修改", command=apply_changes
        )
...
def main():
    ...
    if not target_dir:
        messagebox.showerror(
            "找不到目录",
            "未能在 AppData\\Local\\HT 下找到存放 Windows 配置文件的路径！\n"
            "请确认游戏是否已运行并生成了相关配置文件。",
        )
        sys.exit(0)
